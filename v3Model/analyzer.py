...
def _build_chain():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         """
你是一個資安分析 AI。
你的任務是分析網站內容並回傳 SimplePhishingAnalysis JSON。

【分析原則】
1. 主要依賴你的專業知識和判斷來評估網站安全性。
2. Evidence 僅供參考，不應限制你的判斷。
3. 即使 Evidence 沒有明確標示，只要發現可疑特徵就應該提出。

【explanation 規則】
1. 每點 10-20 字，清楚說明具體風險。
2. 用繁體中文。
3. 最多五個理由 → 用 "、" 連接成單行。
4. 可以參考 Evidence，但主要依賴你的專業判斷。
5. 如果未發現可疑特徵 → explanation = 「未發現可疑特徵」。
6. 可以考慮以下風險特徵（不侷限於此）：
   - 域名註冊時間過短（少於3個月）
   - 網站內容與知名品牌相似度過高
   - 檢測到可疑的URL參數或路徑
